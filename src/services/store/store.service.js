...
const getAllStores = async () => {
  return await Store.find().populate("adminId"); // Middleware automatically filters deleted Stores
};
...
const updateStoreById = async (id, data, avatarLocalPath) => {
  const { name, email, phone, role, store, fcmToken } = data;
  ...
  if (role) {
    ...
    updateData.store = role === "Store" ? store : null; // Update store based on role
  }
...
const softDeleteStoreById = async (id) => {
  ...
  const store = await Store.findById(id); // Corrected to find Store instead of Admin
...
const storeLogin = async (email, password) => {
  ...
  const storeData = await Store.find({ adminId: store._id })
